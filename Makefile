...
clean:
	...
	-rm -f ietf-list-pagination@202?-??-??.yang
	-rm -f includes/example-social@202?-??-??.yang
	-rm -f includes/ex-yang-library@202?-??-??.xml
...
$(next).xml: $(draft).xml
	...
	sed -e"s/YYYY-MM-DD/$(shell date +%Y-%m-%d)/" ietf-list-pagination.yang > ietf-list-pagination\@$(shell date +%Y-%m-%d).yang
	sed -e"s/YYYY-MM-DD/$(shell date +%Y-%m-%d)/" includes/example-social.yang > includes/example-social\@$(shell date +%Y-%m-%d).yang
	sed -e"s/YYYY-MM-DD/$(shell date +%Y-%m-%d)/" includes/ex-yang-library.xml > includes/ex-yang-library\@$(shell date +%Y-%m-%d).xml
